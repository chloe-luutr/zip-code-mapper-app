...
- If "Suggest Optimal ZIPs" is selected, it will recommend a subset of your input ZIPs for efficient 25-mile coverage, displaying the list below the map.
...
def suggest_optimal_zips(gdf_input_zips_geo: gpd.GeoDataFrame, num_recommendations: int, radius_miles: int = 25) -> gpd.GeoDataFrame:
    ...
    if buffer_col not in temp_gdf_for_rec.columns or temp_gdf_for_rec[buffer_col].isnull().all(): 
        st.warning(f"Could not generate valid {radius_miles}-mile buffers for all input ZIPs for recommendation. Skipping.")
        return gpd.GeoDataFrame(columns=['zip'])

    temp_gdf_for_rec = temp_gdf_for_rec[temp_gdf_for_rec.geometry.is_valid & temp_gdf_for_rec.geometry.notna()]
    temp_gdf_for_rec = temp_gdf_for_rec[temp_gdf_for_rec[buffer_col].is_valid & temp_gdf_for_rec[buffer_col].notna()] 
    ...
    # Store original indices, as filtering might change them in temp_gdf_for_rec
    all_input_points_gdf['original_idx'] = all_input_points_gdf.index 
    
    covered_input_zips_original_indices = set() 
    recommended_zips_original_indices = []
    ...
    for i in range(num_recommendations):
        ...
        candidate_indices_in_proj_gdf = [idx for idx in temp_gdf_proj.index if temp_gdf_for_rec.loc[idx].name not in recommended_zips_original_indices]

        if not candidate_indices_in_proj_gdf: break

        for current_proj_idx in candidate_indices_in_proj_gdf: 
            current_buffer_proj = temp_gdf_proj.loc[current_proj_idx, buffer_col]
            if current_buffer_proj is None or current_buffer_proj.is_empty: continue
            
            newly_covered_count = 0
            # Points to check are those whose original indices are not in covered_input_zips_original_indices
            currently_uncovered_points_proj = all_input_points_proj[~all_input_points_proj['original_idx'].isin(covered_input_zips_original_indices)]
            ...
            if newly_covered_count > max_newly_covered:
                ...
                best_candidate_original_idx = temp_gdf_for_rec.loc[current_proj_idx].name # Get original index from source GDF
        
        if best_candidate_original_idx != -1 and (max_newly_covered > 0 or (i == 0 and max_newly_covered >=0)):
            recommended_zips_original_indices.append(best_candidate_original_idx) 
            
            # Find the corresponding row in temp_gdf_proj for the best candidate to get its buffer
            best_candidate_proj_idx = temp_gdf_proj[temp_gdf_for_rec.index == best_candidate_original_idx].index[0]
            best_candidate_buffer_proj = temp_gdf_proj.loc[best_candidate_proj_idx, buffer_col]
            
            points_to_mark_covered_proj = all_input_points_proj[~all_input_points_proj['original_idx'].isin(covered_input_zips_original_indices)]
            if not points_to_mark_covered_proj.empty:
                ...
                if possible_matches_to_mark_indices:
                    ...
                    indices_newly_covered_original = points_to_mark_covered_proj.iloc[possible_matches_to_mark_indices][actually_within_to_mark]['original_idx'].tolist()
                    covered_input_zips_original_indices.update(indices_newly_covered_original)
            covered_input_zips_original_indices.add(best_candidate_original_idx)
        else: break 
            
    if not recommended_zips_original_indices: return gpd.GeoDataFrame(columns=['zip'])
    return gdf_input_zips_geo.loc[recommended_zips_original_indices, ['zip']].reset_index(drop=True)
...
def generate_map_plot(gdf_us, df_input_zips, df_ad_targets, gdf_k12_schools_repo=None, 
    ...
    buffer_plot_styles = { 
        5:  {'facecolor':'red', 'edgecolor':'darkred', 'alpha':0.35, 'linewidth':0.5, 'zorder':3}, # Increased alpha
        10: {'facecolor':'yellow', 'edgecolor':'#B8860B', 'alpha':0.35, 'linewidth':0.5, 'zorder':2}, 
        25: {'facecolor':'lightseagreen', 'edgecolor':'darkcyan', 'alpha':0.3, 'linewidth':0.5, 'zorder':1}
    }
    ...
    if gdf_recommended_zips is not None and not gdf_recommended_zips.empty and \
       ...
       gdf_recommended_buffers_3857['buffer_25_3857'].notna().any():
        ...
        if style:
            rec_style = style.copy(); rec_style['alpha'] = style['alpha'] + 0.1; rec_style['linewidth'] = 1.0; rec_style['zorder'] = 26 
...
zip_code_input_text = st.sidebar.text_area("Paste list of ZIP codes to analyze (comma, space, or newline separated).", height=150, key="zip_input_area_v12")
...
if zip_code_input_text.strip():
    ...
    # Initialize recommended_zips_df_display before the conditional block
    recommended_zips_df_display = gpd.GeoDataFrame(columns=['zip']) # Ensure it's an empty GDF with 'zip'
    ...
    if run_recommendation_toggle and not df_input_zips_data.empty:
        ...
        if not gdf_input_for_rec.empty:
            ...
            # Ensure suggest_optimal_zips returns a GeoDataFrame with at least a 'zip' column
    ...
    if not df_input_zips_data.empty:
        ...
        try:
            map_figure, plotted_schools, final_recommended_zips_for_list_display = generate_map_plot( 
            ...
            # Display the list of recommended ZIPs - use recommended_zips_df_display which is the direct output
            if recommended_zips_df_display is not None and not recommended_zips_df_display.empty:
                ...
                st.dataframe(recommended_zips_df_display[['zip']].reset_index(drop=True), 
                             height=min(300, (len(recommended_zips_df_display) + 1) * 35 + 38),
                             use_container_width=True)
            elif run_recommendation_toggle: 
                 st.warning("Could not generate optimal ZIP recommendations with the current input and settings, or no recommendations found.")
